...
class Qwen_GR00T(baseframework):
    ...
    def predict_action(
        ...
        **kwargs: str,
        ...
        rtc_cfg = dict(rtc or {})
        if "enabled" in rtc_cfg:
            use_rtc = bool(rtc_cfg["enabled"])
        prev_chunk_left_over = rtc_cfg.get("prev_chunk_left_over", prev_chunk_left_over)
        inference_delay = int(rtc_cfg.get("inference_delay", inference_delay))
        if execution_horizon is None:
            execution_horizon = int(getattr(self.action_model.config, "action_horizon", self.action_model.action_horizon))
        execution_horizon = int(rtc_cfg.get("execution_horizon", execution_horizon))
        prefix_attention_schedule = rtc_cfg.get("prefix_attention_schedule", prefix_attention_schedule)
        max_guidance_weight = float(rtc_cfg.get("max_guidance_weight", max_guidance_weight))
        gripper_guidance_scale = float(rtc_cfg.get("gripper_guidance_scale", gripper_guidance_scale))
        ...
        leftover_len = None
        with torch.autocast("cuda", dtype=torch.float32):
            if use_rtc and prev_chunk_left_over is not None:
                with torch.enable_grad():
                    pred_actions = self.action_model.predict_action_rtc(
                        ...
                        gripper_guidance_scale=gripper_guidance_scale,
                    )
                rtc_meta = getattr(self.action_model, "_last_rtc_metadata", {})
                rtc_applied = bool(rtc_meta.get("applied", False))
                leftover_len = rtc_meta.get("leftover_len")
            else:
                pred_actions = self.action_model.predict_action(last_hidden.detach(), state)  # (B, chunk_len, action_dim)
        ...
        return {
            ...
            "rtc": {
                "applied": rtc_applied,
                "chunk_size": int(normalized_actions.shape[1]),
                "action_dim": int(normalized_actions.shape[2]),
